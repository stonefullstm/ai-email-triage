...
class EmbeddingLayer(ClassifierLayer):
    def __init__(self, encoder, top_k: int = 3):
        """
        encoder: qualquer objeto com método encode(text: str) -> np.ndarray
                 ex: SentenceTransformer, modelo local, API externa
        top_k:   quantos vizinhos considerar para votação por maioria
        """
    ...
    def add_example(self, email: EmailInput, label: str) -> None:
        """Adiciona um exemplo anotado ao banco de referência."""
